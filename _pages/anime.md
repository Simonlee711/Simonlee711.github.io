...
Artificial Intelligence has widely slipped under the cracks of how appparent it is within our daily lives. One of the most fascinating applications of AI has been within the realm of recommendation systems.
...
We take what is called a **transformer model** specifically the BERT model to perform this operation. Google has been a big force in the field of Natural language processing and has developed numerous models that are publicly available at [huggingface](https://huggingface.co/). The company's most recent success has been the DALL-E image generator. Because I wasn't the one to design the model myself, I would prefer we get some help from wikipedia to explain what exactly the BERT model does. Wikipedia says the following:
